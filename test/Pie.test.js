...
contract("Pie", (accounts) => {
    ...
    it("adding bakers", async () => {
        await utils.shouldThrow(pie.addBaker(baker1, { from: alice }), "PieRoles: allowed only for Chef");
        ...
        await utils.shouldThrow(pie.addBaker(baker4, { from: bob }), "PieRoles: maximum number of bakers reached");
    });
    ...
    it("removing bakers", async () => {
        ...
        await utils.shouldThrow(pie.removeBaker(baker1, { from: alice }), "PieRoles: allowed only for Chef");
    ...
    it("reassigning Chef role", async () => {
        await utils.shouldThrow(pie.reassignChef(baker1, { from: alice }), "PieRoles: allowed only for Chef");
        await utils.shouldThrow(pie.reassignChef(
            "0x0000000000000000000000000000000000000000", { from: bob }),
        ...
        await utils.shouldThrow(pie.addBaker(baker2, { from: bob }), "PieRoles: allowed only for Chef");
    });
    ...
    it("closing/opening of kitchen", async () => {
        ...
        await utils.shouldThrow(pie.bakePies(BigInt(2e18), { from: baker1 }), "Pie: kitchen must be opened");
        ...
        await utils.shouldThrow(pie.closeKitchen({ from: alice }), "PieRoles: allowed only for Chef");
    });

    it("baking of pies", async () => {
        await utils.shouldThrow(pie.bakePies(BigInt(2e18), { from: baker1 }), "PieRoles: allowed only for Baker");
        await pie.addBaker(baker1, { from: bob });
